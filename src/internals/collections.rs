...
pub mod atomic_ringbuffer {
    ...
    pub const WAIT_RETRY_CAP: u16 = 512;
    ...
    impl<'a> AtomicRingBuffer {
        ...
        /// and its not possible to allocate a slot in time.
        ...
        /// Returns the current head from the buffer and moves the pointer
        /// forward.
        ///
        /// It first checks to see if the buffer is not empty by loading the
        /// `has_message` flag. Then it creates a Block with type T from
        /// [`MatrixHandler`] with the current head, read the value from it,
        /// moves the tail forward, and returns the data already typed to
        /// the caller. If the buffer is empty after moving the tail, set
        /// the `has_message` flag back to false.
        ///
        /// # Returns
        ...
        pub fn dequeue<T>(&self) -> Option<&'a T> {
            ...
            let block = crate::handlers::Block::<T>::from_offset(current_tail);
