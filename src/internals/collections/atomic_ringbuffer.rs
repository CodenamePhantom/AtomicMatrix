...
/// # Note
///
/// All states should be declared with an unique number so logics don't overlap.
...
pub enum Behaviour {
    ...
    Overwrite,
    /// Wait for an amount of time specified by the user before retrying.
    Wait(u32),
}
safe_shm!(Behaviour); // This marks the enum as SHM safe.
...
impl<'a, T: Default + SafeSHM, const N: usize> AtomicRingBuffer<T, N> {
    ...
    ///
    /// # Returns
    /// An optional reference to the written buffer in the matrix. Returns None if the allocation
    /// fails.
    ...
    ///
    /// # Returns
    /// Either a life time specified reference to the block data, or None if the block is empty
    ...
    ///
    /// # Returns
    /// Either a reference to the block data, or None if the block is empty
    ...
    /// Returns the absolute address of the ringbuffer inside the matrix.
