...
pub mod core {
    ...
    impl AtomicMatrix {
        ...
        pub fn bootstrap(id: Option<Uuid>, size: usize) -> Result<crate::handlers::matrix_handler::MatrixHandler, String> {
            ...
            if init_guard.compare_exchange(SYS_UNINITIALIZED, SYS_FORMATTING, Ordering::SeqCst, Ordering::Relaxed).is_ok() {
            ...
            Ok(crate::handlers::matrix_handler::MatrixHandler {
                matrix: unsafe { &mut *matrix_ptr },
                mmap,
            })
        }

        pub fn sectorize(&self, base_ptr: *mut u8, total_file_size: usize, mut small_percent: u8, mut medium_percent: u8) -> Result<(), String> {
            ...
            if total_file_size < 5 * 1024 * 1024 {
                small_percent = 30; medium_percent = 70;
            }

            let small_size = (((usable_space as u64) * (small_percent as u64)) / 100) as u32 & !15;
            let medium_size = (((usable_space as u64) * (medium_percent as u64)) / 100) as u32 & !15;
                    ...
                    if let Ok(offset) = self.remove_free_block(base_ptr, f_fl, f_sl) {
                        unsafe {
                            ...
                            if total_size >= size + 32 {
                                ...
                                let next_h = &mut *(base_ptr.add(next_off as usize) as *mut BlockHeader);
        ...
        fn find_suitable_block(&self, fl: u32, sl: u32) -> Option<(u32, u32)> {
            ...
            if m_sl != 0 { return Some((fl, m_sl.trailing_zeros())); }
            ...
            if m_fl != 0 {
                ...
                if f_fl < 32 {
                    ...
                    if s_map != 0 { return Some((f_fl, s_map.trailing_zeros())); }
                }
            }
            None
        }
        ...
        fn remove_free_block(&self, base_ptr: *mut u8, fl: u32, sl: u32) -> Result<u32, String> {
            ...
            loop {
                ...
                if off == 0 { return Err("Empty".into()); }
                let next = unsafe { (*(base_ptr.add(off as usize) as *const BlockHeader)).next_free.load(Ordering::Acquire) };
        ...
        fn insert_free_block(&self, base_ptr: *mut u8, offset: u32, fl: u32, sl: u32) {
            ...
            unsafe {
                ...
                loop {
                    ...
                    if head.compare_exchange(old, offset, Ordering::SeqCst, Ordering::Relaxed).is_ok() { break; }
                    std::hint::spin_loop();
                }
            }
        ...
        fn create_and_insert_sector(&self, base_ptr: *mut u8, offset: u32, size: u32, prev: u32, fl: u32, sl: u32) {
            unsafe {
                let h = &mut *(base_ptr.add(offset as usize) as *mut BlockHeader);
